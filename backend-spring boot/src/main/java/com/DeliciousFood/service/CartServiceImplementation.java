...
import com.DeliciousFood.repository.CartItemRepository;
import com.DeliciousFood.repository.CartRepository;
import com.DeliciousFood.repository.foodRepository;
...
public class CartServiceImplementation implements CartSerive {
	...
	public CartItem updateCartItemQuantity(Long cartItemId,int quantity) throws CartItemException {
		...
		cartItem.get().setTotalPrice((((Food) cartItem.get().getFood()).getPrice()*quantity));
	...
	public Long calculateCartTotals(Cart cart) throws UserException {
		...
		Long total = 0L;
		for (CartItem cartItem : cart.getItems()) {
			total += ((Food) cartItem.getFood()).getPrice() * cartItem.getQuantity();
		}
		return total;
	}
